 ...
 * Claude CLI Adapter
 *
 * Integrates with Anthropic's Claude CLI tool
...
export class ClaudeCLIAdapter extends BaseCLIAdapter {
  readonly provider: CLIProvider = 'claude';
  ...
  async getVersion(): Promise<string> {
    try {
      ...
      throw new Error(
        `Failed to get Claude CLI version: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
...
/**
 * Create a Claude CLI adapter instance
 */
