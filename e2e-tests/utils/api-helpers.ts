...
export function validateResponseSchema<T>(
  ...
  for (const field of requiredFields) {
    expect(response.data as Record<string, unknown>).toHaveProperty(field);
  }
...
export async function testCRUDOperations(
  ...
  // UPDATE
  const updateResponse = await testPUT(context, `${baseUrl}/${resourceId}`, updateData);
  ...
  const deletedResponse = await apiRequest(context, 'GET', `${baseUrl}/${resourceId}`);
...
export async function waitForMultipleAPICalls(
  ...
  const promises = patterns.map((pattern) =>
    page.waitForResponse(
      (resp) =>
        typeof pattern === 'string' ? resp.url().includes(pattern) : pattern.test(resp.url()),
      { timeout }
    )
  );
...
export async function simpleLoadTest(
  ...
  for (let i = 0; i < requests; i += concurrency) {
    const batch = Array.from({ length: Math.min(concurrency, requests - i) }).map(async () => {
      const start = Date.now();
      try {
