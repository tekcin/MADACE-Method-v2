 ...
 *
 * Tests agent listing page, agent detail page, navigation,
 * and agent display functionality for all 5 MAM agents.
 */
...
test.describe('Agent Management', () => {
  ...
  test('should display all 5 MAM agents', async ({ page }) => {
    ...
    for (const agent of expectedAgents) {
      ...
      const cardText = await getTextContent(agentCard);
      expect(cardText).toContain(agent.name);
    }
  });
  ...
  test('should display full agent details', async ({ page }) => {
    // Navigate to PM agent detail by clicking card
    ...
    // Verify module badge exists
    const moduleBadge = page.locator('span.rounded-full').first();
    if (await isVisible(moduleBadge)) {
      const badgeText = await getTextContent(moduleBadge);
      expect(badgeText.length).toBeGreaterThan(0);
    }

    // Verify agent version is shown
    const versionText = page.locator('p.text-gray-600').first();
    await expect(versionText).toBeVisible();

    // Verify agent persona section exists
    const personaSection = page.locator('h2:has-text("Persona")');
    if (await isVisible(personaSection)) {
      await expect(personaSection).toBeVisible();
      const personaContent = personaSection.locator('..').locator('pre');
      await expect(personaContent).toBeVisible();
    }

    // Verify agent menu section exists
    const menuSection = page.locator('h2:has-text("Menu")');
    if (await isVisible(menuSection)) {
      await expect(menuSection).toBeVisible();
      const menuContent = menuSection.locator('..').locator('pre');
      await expect(menuContent).toBeVisible();
    }

    // Verify agent prompts section exists
    const promptsSection = page.locator('h2:has-text("Prompts")');
    if (await isVisible(promptsSection)) {
      await expect(promptsSection).toBeVisible();
      const promptsContent = promptsSection.locator('..').locator('pre');
      await expect(promptsContent).toBeVisible();
    }

    // Verify actions section exists
    const actionsSection = page.locator('h2:has-text("Actions")');
    if (await isVisible(actionsSection)) {
  ...
  test('should navigate back to agents list', async ({ page }) => {
    ...
    await clickAndWait(backButton, { waitForNavigation: true });
  ...
  test('should be responsive on mobile', async ({ page }) => {
    ...
    // Agent card should not exceed viewport width (with padding)
  ...
  test('should display module filter', async ({ page }) => {
    ...
    if (await isVisible(moduleFilter)) {
      ...
      const filterOptions = moduleFilter.locator('option');
      const optionCount = await filterOptions.count();
      expect(optionCount).toBeGreaterThan(0);

      // Verify "All Agents" option exists
  ...
  test('should not have console errors', async ({ page }) => {
    ...
    const firstAgent = page.locator('[aria-label*="Select"][aria-label*="agent"]').first();
    if (await isVisible(firstAgent)) {
      await clickAndWait(firstAgent, { waitForNavigation: true });
      await waitForNetworkIdle(page);
    }

    // Filter out expected errors (e.g., 404s from optional resources)
  ...
  test('should handle loading state', async ({ page }) => {
    ...
    const loadingText = page.locator('text=/Loading agents/i').first();

    const hasLoadingIndicator = await isVisible(loadingIndicator, 1000);
    const hasLoadingText = await isVisible(loadingText, 1000);

    // At least one loading indicator should appear
    expect(hasLoadingIndicator || hasLoadingText).toBeTruthy();
  });
