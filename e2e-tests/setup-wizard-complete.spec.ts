...
test.describe('Setup Wizard Complete Flow', () => {
  ...
  test('should complete full setup wizard flow', async ({ page }) => {
    ...
    // Click Finish and wait for config save API call
    const savePromise = waitForAPIResponse(page, '/api/config', {
      method: 'POST',
      timeout: 10000,
    });
  ...
  test('should show loading state when saving configuration', async ({ page }) => {
    ...
    // Wait for save to complete (accept alert if it appears)
    page.once('dialog', async (dialog) => {
      await dialog.accept();
    });
